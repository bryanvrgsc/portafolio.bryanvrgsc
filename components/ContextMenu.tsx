...
const ContextMenu: React.FC<ContextMenuProps> = ({ x: initialX, y: initialY, onClose, items }) => {
  ...
  return (
    <motion.div
      ...
      className="tahoe-context-menu fixed w-56 rounded-2xl py-1.5 z-[5000] select-none"
    >
      {items.map((item, i) => (
        <React.Fragment key={i}>
          <button
            ...
            className="tahoe-context-menu-item w-full px-3 py-2 text-left text-[13px] flex items-center transition-colors rounded-xl"
          >
            {item.label}
          </button>
          {item.divider && <div className="tahoe-menu-separator h-px my-1 mx-2" />}
        </React.Fragment>
      ))}
    </motion.div>
  );
